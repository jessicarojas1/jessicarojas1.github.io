...
<div class="card" style="margin:18px 0">
  <div class="card-body">
    <form method="GET" action="/documents" class="form-row" style="align-items:flex-end;gap:12px;flex-wrap:wrap">
      ...
      <div class="form-group" style="margin:0"><label class="form-label">Status</label><select name="status" class="form-select"><option value="">All</option><?php foreach (['draft','in_review','approved','published','superseded','retired','expired','rejected','archived','obsolete'] as $st): ?><option value="<?= $st ?>" <?= ($_GET['status'] ?? '')===$st?'selected':'' ?>><?= ucfirst(str_replace('_',' ',$st)) ?></option><?php endforeach; ?></select></div>
