  ...
  const rpt = el(`<div class="print-report">
    <div class="report-header">
      <div style="display:flex;align-items:center;gap:10px">
        ${logo ? `<img src="${esc(logo)}" alt="logo" style="height:40px;object-fit:contain">` : ""}
        <div><h1>${esc(brand.org_name || "AeroMarkup")} — Engineering Markup Report</h1>
...
export async function renderSettings(host) {
  ...
  const curCls = await getClassification();
  host.innerHTML = `${header("Settings", "Branding and application preferences")}
    ${card("Branding", `
      <div class="form-grid">
        <div class="field" style="grid-column:1/-1"><label>Organization / App Name</label>
          <input class="input" data-name="org_name" value="${esc(b.org_name || "")}"></div>
        <div class="field" style="grid-column:1/-1"><label>Logo URL</label>
          <input class="input" data-name="logo_url" value="${esc(b.logo_url || "")}" placeholder="https://example.gov/logo.png  or  data:image/png;base64,…">
          <div class="help">Paste a logo URL, or upload a file below (stored on-device so it works offline).</div></div>
        <div class="field"><label>Upload Logo File</label><input type="file" accept="image/*" class="input" data-logo-file></div>
        <div class="field"><label>Accent Color</label><input type="color" class="input" data-name="accent" value="${esc(b.accent || "#ff5811")}"></div>
      </div>
      <div class="signature-box" style="margin-top:var(--s4);display:flex;align-items:center;gap:12px">
        <div class="kpi-label">Preview</div>
        <span class="brand-mark" data-preview-mark style="width:28px;height:28px;display:grid;place-items:center">${b.logo_url && safeLogo(b.logo_url) ? `<img src="${esc(safeLogo(b.logo_url))}" style="width:24px;height:24px;object-fit:contain;border-radius:4px">` : icon("plane", 20)}</span>
        <strong data-preview-name>${esc(b.org_name || "AeroMarkup")}</strong>
      </div>`,
      `<button class="btn btn-primary" data-save-brand>Save Branding</button>`)}
    ${card("Document Defaults", `
      <div class="form-grid">
        <div class="field"><label>Default Classification</label>
          <select class="select" data-name="classification">
            ${["CUI", "UNCLASSIFIED", "CUI//SP-PROPIN", "UNCLASS//FOUO"].map((c) => `<option ${curCls === c ? "selected" : ""}>${c}</option>`).join("")}
          </select></div>
      </div>`,
      `<button class="btn btn-outline" data-save-cls>Apply</button>`)}
    ${card("Storage", `<p class="muted">All data is stored on this device (IndexedDB) and synced on demand.
      This keeps AeroMarkup usable offline and in air-gapped environments.</p>
      <button class="btn btn-danger" data-reset>Clear local demo data &amp; reseed</button>`)}`;

  // live preview
  const refreshPreview = () => {
    const url = safeLogo($('[data-name="logo_url"]', host).value);
    $("[data-preview-name]", host).textContent = $('[data-name="org_name"]', host).value || "AeroMarkup";
    $("[data-preview-mark]", host).innerHTML = url
      ? `<img src="${esc(url)}" style="width:24px;height:24px;object-fit:contain;border-radius:4px">` : icon("plane", 20);
  };
  $('[data-name="org_name"]', host).addEventListener("input", refreshPreview);
  $('[data-name="logo_url"]', host).addEventListener("input", refreshPreview);

  $("[data-logo-file]", host).addEventListener("change", (e) => {
    const f = e.target.files[0]; if (!f) return;
    const r = new FileReader();
    r.onload = () => { $('[data-name="logo_url"]', host).value = r.result; refreshPreview(); };
    r.readAsDataURL(f);
  });

  $("[data-save-brand]", host).addEventListener("click", async () => {
    const v = formValues(host);
    await setBranding({ org_name: v.org_name, logo_url: v.logo_url, accent: v.accent });
    await logAudit("settings.branding", "settings", null, { org_name: v.org_name });
    window.dispatchEvent(new CustomEvent("am:branding-changed"));
    toast("Branding saved", "success");
  });
  $("[data-save-cls]", host).addEventListener("click", async () => {
    const { setClassification } = await import("./session.js");
    const v = formValues(host);
    await setClassification(v.classification);
    document.querySelectorAll("[data-cui]").forEach((x) => (x.textContent = v.classification));
    toast("Default classification applied", "success");
  });
  $("[data-reset]", host).addEventListener("click", async () => {
    if (!confirm("Clear all local AeroMarkup data on this device and reseed demo content?")) return;
    indexedDB.deleteDatabase("aeromarkup");
    toast("Cleared. Reloading…", "info");
