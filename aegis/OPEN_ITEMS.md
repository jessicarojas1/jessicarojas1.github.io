...
| TD-1a (P1) | **Concrete runtime-only schema not captured as migrations** (surfaced by the Phase 21 audit; same class as the fixed `kris.direction`/migration 037). A DB built purely from migrations + `schema.full.sql` is missing these (web boot self-heals via the TD-1 block, but cron-only/fresh contexts do not): **columns** `change_requests.implemented_at`, `assets.created_by`, `compliance_objectives.additional_information`, `users.sessions_revoked_at`/`force_password_change`/`password_changed_at`, `incidents.phi_involved`/`breach_notification_required`/`breach_notification_sent_at`/`root_cause`, `issues.resolution`, `audit_findings.audit_id`; **tables** `totp_used_codes`, `ai_inference_log`, `password_history`; plus runtime re-creation of `incidents_status_check`/`issues_status_check` | Fresh-install/cron-only DBs miss these; `password_history` in particular backs `ProfileController::changePassword` | Promote each into a numbered migration (like 037), register in `install.php`, and add to `schema.sql`/`schema.full.sql`; then delete the runtime guards |
