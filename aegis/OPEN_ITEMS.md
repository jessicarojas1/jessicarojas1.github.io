...
|---|---|---|---|
| TD-1 (P3, **reassessed + slimmed** — Phase 26) | Correction: the `index.php` runtime schema block is **version-gated** (`schema_runtime_state` — steady state is one indexed SELECT then the whole block is skipped), not per-request, and is a deliberate **fail-safe self-heal** (marker written last). So the original "per-request latency" premise was inaccurate. Phase 26 removed the block's **dead-module resurrections** — it was re-creating `account_reviews`/`account_review_items` and ALTERing `change_requests`, all **dropped by migration 032** (no controller/route/view) — zombie schema. `schema.full.sql` also had `account_reviews` wrongly re-added in Phase 22; removed. What remains in the block is redundant-but-harmless self-heal DDL for **live** tables (all covered by `schema.sql` + migrations, now CI-guarded by `verify_fresh_schema.php`) plus data seeds/backfills | Drift risk only; no runtime cost of note. The block is now free of dead code | Optional future cleanup: drop the live-table CREATE/ALTER self-heal too once confident every deploy runs `install.php` (migrations). Low priority — the block is a safety net, not a defect |
