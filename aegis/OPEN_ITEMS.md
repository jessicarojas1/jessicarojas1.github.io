...
| TD-1b (P1, **new** — Phase 22 discovery) | A **truly fresh `install.php` does not build a complete/consistent schema** independent of the `index.php` runtime block: `change_requests` is created by migration 003 then dropped by 032 (yet code/runtime still references it); `install.php` logs `column "risk_tier" does not exist` (an index build over a vendors column that is itself runtime-only); migration `006` runs before `008` and references `user_notification_prefs` before it exists (caught as a warning). The app only works because the `index.php` runtime block heals the DB on first web boot | Cron-only / migrate-only / air-gapped installs can be subtly incomplete until a web request lands; warnings mask real gaps | Dedicated remediation: fix migration ordering (008 before 006's dependency), remove dead `risk_tier` index / `change_requests` references, make `install.php` fail loudly on migration errors instead of warning-and-continue |
| TD-4 (P1) | Schema is expressed in **three** places: `database/schema.sql`, `database/migrations/*` (37 files), and the TD-1 runtime block | Drift risk; harder to satisfy the "schema.sql reflects all migrations" rule | Treat migrations as canonical; regenerate/validate `schema.full.sql` in CI; shrink `schema.sql` to a pointer or keep it validated |
| TD-4a (P2) | **`schema.full.sql` stale vs migrations** (Phase 21 audit): `account_reviews` + `account_review_items` (migration 012), `incident_updates.update_type`, and `issue_updates.update_type` exist via migrations/`install.php` but are absent from `schema.full.sql` | Reference dump under-represents the real schema (rule-3 violation) | Regenerate `schema.full.sql` from a migrated DB; add a CI check that diffs it against a fresh migrate |
