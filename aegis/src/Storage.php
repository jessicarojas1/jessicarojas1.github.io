...
class Storage {
    ...
    public static function get(string $key): string|false {
        ...
        return file_get_contents(AEGIS_ROOT . '/' . $key);
    }
    ...
    private static function s3Request(
        ...
        if ($responseCode >= 400) {
            error_log("AEGIS Storage S3 error {$responseCode} on {$method} {$key}: {$responseBody}");
        }
