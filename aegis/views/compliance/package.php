...
<div class="page-header">
  ...
  <div class="page-actions">
    ...
    <button class="btn btn-ghost" data-click="openModal" data-arg="edit-pkg"><i class="bi bi-pencil-fill"></i> Edit</button>
...
<div id="bulk-bar" style="display:none;position:sticky;top:0;z-index:200;background:var(--primary-dark,#1c2a1b);color:#fff;
  ...
  <button class="bulk-status-btn" data-status="not_started"   style="background:var(--text-muted);color:#fff;border:none;border-radius:6px;padding:6px 12px;font-size:12px;font-weight:600;cursor:pointer"><i class="bi bi-circle"></i> Not Started</button>
...
<div class="filter-bar card" id="domains">
  <form method="GET" class="filter-form">
    ...
    <button type="button" class="btn btn-primary btn-sm" data-click="openModal" data-arg="add-domain">
...
<div class="card" style="text-align:center;padding:40px 20px;color:var(--text-muted)">
  <i class="bi bi-diagram-3" style="font-size:2.5rem;display:block;margin-bottom:12px;color:var(--text-muted)"></i>
...
<div class="domain-block card" id="domain-<?= $domain['id'] ?>">
  <div class="domain-header" data-click="toggleDomain" data-arg="<?= $domain['id'] ?>" style="cursor:pointer">
    ...
    <div class="domain-header-right" style="display:flex;align-items:center;gap:8px">
      ...
      <button class="btn btn-ghost btn-sm" title="Add Control"
              data-click="openModal" data-arg="add-ctrl-<?= $domain['id'] ?>" data-stop-prop>
        <i class="bi bi-plus-lg"></i> Add Control
      </button>
      <button class="btn btn-ghost btn-sm" title="Edit Domain"
              data-click="openModal" data-arg="edit-domain-<?= $domain['id'] ?>" data-stop-prop>
  ...
  <div class="domain-controls" id="controls-<?= $domain['id'] ?>" style="display:none">
      ...
      <div class="control-row" data-status="<?= Security::h($implStatus) ?>" data-domain="<?= $domain['id'] ?>">
        ...
        <div style="display:flex;gap:4px;align-items:center">
          ...
          <button class="btn btn-ghost btn-sm" title="Edit control details"
                  data-click="openModal" data-arg="edit-ctrl-<?= $ctrl['id'] ?>">
    ...
    <div style="padding:10px 16px;border-top:1px solid var(--border-light);background:var(--bg-secondary)">
      <button class="btn btn-ghost btn-sm" data-click="openModal" data-arg="add-ctrl-<?= $domain['id'] ?>">
...
// Stop domain-select-all checkbox clicks from toggling domain accordion
document.addEventListener('click', function(e) {
  var cb = e.target.closest('.domain-select-all');
    ...
    <div class="card-body" style="display:flex;flex-direction:column;gap:16px;max-height:70vh;overflow-y:auto;padding:20px">
      ...
      <div class="form-group" style="margin:0">
        ...
        <div style="display:flex;flex-direction:column;gap:8px">
          <?php foreach ([
            ['not_started',   'circle',              'var(--text-muted)', 'Not Started',   'No implementation work has begun'],
            ...
            ['not_applicable','slash-circle-fill',   'var(--text-muted)', 'Not Applicable','This control does not apply'],
