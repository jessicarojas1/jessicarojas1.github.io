  ...
  <div style="display:flex;flex-direction:column;gap:20px;">
    ...
    <div class="card">
      <div class="card-header" style="display:flex;align-items:center;justify-content:space-between;">
        ...
        <button id="btnOpenAddPkg" class="btn btn-sm btn-primary"><i class="bi bi-plus-lg"></i> Add Package</button>
        <?php endif; ?>
      </div>
      ...
      <div class="card-body" style="text-align:center;padding:30px;">
        ...
        <?php if (!empty($allPackages)): ?>
        <button id="btnOpenAddPkg2" class="btn btn-primary btn-sm">Add Package</button>
        <?php endif; ?>
      </div>
      <?php else: ?>
      <table class="table">
...
<div id="editModal" style="display:none;position:fixed;inset:0;z-index:1000;align-items:center;justify-content:center;background:rgba(0,0,0,0.5);">
  <div style="background:var(--card-bg);border-radius:12px;padding:28px;width:780px;max-height:90vh;overflow-y:auto;max-width:95vw;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
      ...
      <button id="btnCloseEdit" style="background:none;border:none;cursor:pointer;font-size:1.25rem;color:var(--text-muted);"><i class="bi bi-x-lg"></i></button>
    </div>
...
<div id="addPkgModal" style="display:none;position:fixed;inset:0;z-index:1000;align-items:center;justify-content:center;background:rgba(0,0,0,0.5);">
  <div style="background:var(--card-bg);border-radius:12px;padding:28px;width:480px;max-width:95vw;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
      ...
      <button id="btnClosePkg" style="background:none;border:none;cursor:pointer;font-size:1.25rem;color:var(--text-muted);"><i class="bi bi-x-lg"></i></button>
    </div>
    <form method="POST" action="/ssp/<?= (int)$plan['id'] ?>/add-package">
      ...
      <div style="display:flex;gap:10px;margin-top:16px;">
        ...
        <button type="button" id="btnCancelPkg" class="btn btn-secondary">Cancel</button>
      </div>
    </form>
  </div>
</div>
...
(function() {
  ...
  var pkgModal  = document.getElementById('addPkgModal');
  ...
  if (editModal) editModal.addEventListener('click', function(e) { if (e.target === editModal) close(editModal); });

  var btnOpenPkg  = document.getElementById('btnOpenAddPkg');
  var btnOpenPkg2 = document.getElementById('btnOpenAddPkg2');
  var btnClosePkg = document.getElementById('btnClosePkg');
  var btnCancelPkg= document.getElementById('btnCancelPkg');
  if (btnOpenPkg)  btnOpenPkg.addEventListener('click',   function() { open(pkgModal); });
  if (btnOpenPkg2) btnOpenPkg2.addEventListener('click',  function() { open(pkgModal); });
  if (btnClosePkg) btnClosePkg.addEventListener('click',  function() { close(pkgModal); });
  if (btnCancelPkg)btnCancelPkg.addEventListener('click', function() { close(pkgModal); });
  if (pkgModal)    pkgModal.addEventListener('click', function(e) { if (e.target === pkgModal) close(pkgModal); });
