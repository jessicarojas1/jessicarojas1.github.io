  ...
  <div style="display:flex;flex-direction:column;gap:20px;">
    ...
    <div class="card">
      <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;">
        ...
        <button id="btnOpenTask" class="btn btn-sm btn-secondary"><i class="bi bi-plus-lg"></i> Add Task</button>
      </div>
      <?php if ($taskCount > 0): ?>
    ...
    <div class="card">
      <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;">
        ...
        <button id="btnOpenLink" class="btn btn-sm btn-secondary"><i class="bi bi-link-45deg"></i> Add Link</button>
      </div>
      <?php if (empty($links)): ?>
...
<div id="addTaskModal" style="display:none;position:fixed;inset:0;z-index:1000;align-items:center;justify-content:center;background:rgba(0,0,0,0.5);">
  <div style="background:var(--card-bg);border-radius:12px;padding:28px;width:480px;max-width:95vw;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
      ...
      <button id="btnCloseTask" style="background:none;border:none;cursor:pointer;font-size:1.25rem;"><i class="bi bi-x-lg"></i></button>
    </div>
    <form method="POST" action="/projects/<?= (int)$project['id'] ?>/task/add">
      ...
      <div style="display:flex;gap:10px;margin-top:16px;">
        ...
        <button type="button" id="btnCancelTask" class="btn btn-secondary">Cancel</button>
      </div>
    </form>
  </div>
</div>
...
<div id="addLinkModal" style="display:none;position:fixed;inset:0;z-index:1000;align-items:center;justify-content:center;background:rgba(0,0,0,0.5);">
  <div style="background:var(--card-bg);border-radius:12px;padding:28px;width:400px;max-width:95vw;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
      ...
      <button id="btnCloseLink" style="background:none;border:none;cursor:pointer;font-size:1.25rem;"><i class="bi bi-x-lg"></i></button>
    </div>
    <form method="POST" action="/projects/<?= (int)$project['id'] ?>/link/add">
      ...
      <div style="display:flex;gap:10px;margin-top:16px;">
        ...
        <button type="button" id="btnCancelLink" class="btn btn-secondary">Cancel</button>
      </div>
    </form>
  </div>
</div>
...
(function() {
  ...
  editModal.addEventListener('click', function(e) { if (e.target === editModal) close(editModal); });

  document.getElementById('btnOpenTask').addEventListener('click',   function() { open(taskModal); });
  document.getElementById('btnCloseTask').addEventListener('click',  function() { close(taskModal); });
  document.getElementById('btnCancelTask').addEventListener('click', function() { close(taskModal); });
  taskModal.addEventListener('click', function(e) { if (e.target === taskModal) close(taskModal); });

  document.getElementById('btnOpenLink').addEventListener('click',   function() { open(linkModal); });
  document.getElementById('btnCloseLink').addEventListener('click',  function() { close(linkModal); });
  document.getElementById('btnCancelLink').addEventListener('click', function() { close(linkModal); });
  linkModal.addEventListener('click', function(e) { if (e.target === linkModal) close(linkModal); });
