...
- **Expected behavior:** `Security::setSecurityHeaders()` (`Security.php:337-380`) emits a CSP with `script-src 'self' 'nonce-…'` and `style-src 'self' 'unsafe-inline'` (no external origin in any directive — all assets vendored locally; no `unsafe-inline` for scripts), `frame-ancestors 'none'`, `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, plus `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`, COOP/CORP, `Permissions-Policy`, and HSTS over HTTPS.
