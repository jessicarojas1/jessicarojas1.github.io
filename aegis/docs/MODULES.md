...
- **Review-overdue monitoring (Phase 18):** surfaces System Security Plans whose own review / re-authorization cadence has lapsed — i.e. the SSP document's `next_review_date` has passed. This is **distinct** from `policy_review_due` (policy documents) and the compliance control re-test cadence; it tracks the SSP's own re-authorization due date. The pure helper `SSPController::reviewStatus(?string $nextReviewDate)` (`public static`) returns `none` (no/unparseable date), `overdue` (past), `due` (within 30 days), or `ok`. `views/ssp/index.php` now shows a per-plan "Review overdue" (red) / "Review due" (amber) badge next to the plan title via the helper — the SSP list did not previously surface `next_review_date` at all. `scripts/send_notifications.php` adds an `ssp_review_overdue` section that alerts the SSP **owner** (`ssp_plans.created_by`) when `next_review_date` is past today, throttled to one reminder per SSP per 7 days. Registered as a user-toggleable preference (`ProfileController::$types` + `views/profile/notifications.php`, label "SSP review overdue"). **No schema migration** — reuses the existing `ssp_plans.next_review_date` / `created_by` columns. Covered by `tests/test_ssp_review.php` (unit — review-status buckets + predicate drift guard) and `tests/integration/ssp_review_db.php` (DB-level overdue predicate, active-owner join, future/no-date/inactive-owner exclusion, ssp→control_statements cascade; wired into `.github/workflows/aegis-integration.yml`).
...
- **Review-overdue monitoring (Phase 19):** first coverage for the asset inventory's **annual review lifecycle** — surfaces assets that have not had their yearly review. The pure helper `AssetController::reviewStatus(?string $lastReviewed, string $status, ?string $createdAt, int $intervalDays = 365)` (`public static`) returns `none` (decommissioned assets never need review), `overdue` (elapsed > interval), `due` (within 30 days of the interval), or `ok`; the baseline is `last_reviewed`, **falling back to** the asset's `created_at` when never reviewed, and the cadence is the `REVIEW_INTERVAL_DAYS` const = 365 (annual). `views/assets/index.php` now shows a per-asset "Review overdue" (red) / "Review due" (amber) badge next to the asset name via the helper (the list did not previously surface `last_reviewed`). `scripts/send_notifications.php` adds an `asset_review_overdue` section that alerts the asset **owner** (`assets.owner_id`) when elapsed days since `last_reviewed` (or `created_at` if never reviewed) exceeds 365 and the asset is not decommissioned, throttled to one reminder per asset per 7 days; the type is registered as a user-toggleable preference (`ProfileController::$types` + `views/profile/notifications.php`, label "Asset review overdue"). **No schema migration** — reuses the existing `assets.last_reviewed` / `status` / `owner_id` / `created_at` columns. Covered by `tests/test_asset_review.php` (unit, review buckets + never-reviewed fallback + decommissioned exclusion + predicate drift guard) and `tests/integration/asset_review_db.php` (DB-level annual-cadence predicate, owner join, fresh/decommissioned/inactive-owner/owner-less exclusion; wired into `.github/workflows/aegis-integration.yml`).
...
- **Schedule-overdue monitoring (Phase 16):** surfaces scheduled **audits** whose planned date has **passed while the audit is still open** — the audit event that was scheduled but never carried out. This is **distinct** from `finding_remediation_overdue` (Phase 11), which tracks remediation deadlines on `audit_findings`; this one is on the `audits` table itself. The pure helper `AuditController::scheduleStatus(?string $scheduledDate, string $status, ?string $completedDate)` (`public static`) returns `none` (a settled status `completed`/`cancelled` — shared via the `TERMINAL_STATUSES` const — **or** a set `completed_date`, **or** no scheduled date), `overdue` (scheduled date passed and still open), `due` (scheduled within 14 days), or `ok`. `views/audit/index.php` now drives its per-row schedule badge through the helper: it previously only flagged `status='planned'` audits as "Overdue"; it now covers **all** open statuses (`planned`/`in_progress`/`overdue`), adds a "Due soon" (amber) badge, and the row-danger highlight follows the helper. `scripts/send_notifications.php` adds an `audit_schedule_overdue` section that emails the assigned **auditor** (`audits.auditor_id`, **falling back to** `audits.created_by`) when `scheduled_date` has passed and the audit is not `completed`/`cancelled` (and has no `completed_date`), throttled to one reminder per audit per 7 days; the type is registered as a user-toggleable preference (`ProfileController` + `views/profile/notifications.php`, label "Audit overdue to start"). **No schema migration** — reuses the existing `audits.scheduled_date` / `status` / `completed_date` / `auditor_id` / `created_by` columns. Covered by `tests/test_audit_schedule.php` (unit, `scheduleStatus` buckets + terminal/completed-date exclusion + predicate drift guard) and `tests/integration/audit_schedule_db.php` (DB-level overdue predicate, owner `COALESCE` fallback, future/completed/cancelled exclusion, audit→`audit_items` cascade; wired into `.github/workflows/aegis-integration.yml`).
...
- **Attestation-overdue monitoring (Phase 17):** surfaces policy-**attestation campaigns** whose `due_date` has passed while still `is_active`, and reminds each active user who has not yet attested. Since there is no per-user assignment table, every active user is expected to attest to an active campaign's policy; **"outstanding"** = an active user with **no** `policy_attestations` row for the campaign's `policy_id`. The pure helper `PolicyController::attestationCampaignStatus(?string $dueDate, bool $isActive)` (`public static`) returns `none` (inactive or no due date), `overdue` (active + past due), `due` (active + within 14 days), or `ok`; `views/policy/attestations.php` now renders the per-campaign due-date badge from it — an **"Overdue"** (red) / **"Due soon"** (amber) badge, replacing the prior ad-hoc overdue marker. `scripts/send_notifications.php` adds a `policy_attestation_overdue` section that alerts **each outstanding active user** — a **CROSS JOIN** of active users with **`NOT EXISTS`** a `policy_attestations` row for the campaign's policy — for an active campaign whose `due_date` has passed, throttled to one reminder per user per campaign per 7 days. Registered as a user-toggleable preference (`ProfileController::$types` + `views/profile/notifications.php`, label "Policy attestation overdue"). **No schema migration** — reuses the existing `policy_attestation_campaigns` / `policy_attestations` columns. Covered by `tests/test_policy_attestation.php` (unit — campaign-status buckets + outstanding-predicate drift guard) and `tests/integration/policy_attestation_db.php` (DB-level outstanding CROSS JOIN/`NOT EXISTS` predicate, attested/inactive-user exclusion, future/inactive-campaign exclusion, policy→campaign+attestation cascade; wired into `.github/workflows/aegis-integration.yml`).
...
### Features
Month grid (`index`) and JSON feed (`feed`). Aggregates: controls due (non-compliant/not_applicable excluded from "due"), policy reviews (published), audit schedules (non-completed), risk-treatment due dates, plus (Phase 20) control re-tests (latest `control_tests.next_test_date` per objective), audit finding remediation deadlines (`audit_findings.deadline`), vendor certification expiries (`vendor_certifications.expiry_date`), vendor contract end dates (`vendor_contracts.end_date`), policy attestation campaign due dates (`policy_attestation_campaigns.due_date`), SSP review dates (`ssp_plans.next_review_date`), asset annual reviews (`assets.last_reviewed` or `created_at` + 365 days), and KRI next-measurement due dates (`kris` last recorded value or `created_at` + the cadence window derived from `frequency`).

- **Compliance-calendar consolidation (Phase 20):** the capstone of the Phase 10–19 GRC monitoring arc — rather than a new page, it **extends the existing live Compliance Calendar** (`/calendar`, `CalendarController`, `views/calendar/index.php`) so every cadence signal built across the arc surfaces on one month grid. `CalendarController::getEvents(int $year, int $month)` previously aggregated 4 event types (controls due, policy reviews, audits, risk treatments); Phase 20 adds **8 more categories** — control re-tests (`control_retest`), audit finding remediation deadlines (`finding`), vendor certification expiries (`vendor_cert`), vendor contract end dates (`vendor_contract`), policy attestation campaign due dates (`attestation`), SSP review dates (`ssp_review`), asset annual reviews (`asset_review`), and KRI next-measurement due dates (`kri_measurement`). `getEvents()` was promoted from `private static` to **`public static`** so it is integration-testable (it is a helper, not a routed action — `check_route_auth` skips static methods); auth is unchanged (both `index`/`feed` remain gated by `Auth::requirePermission('risk.view')`). `views/calendar/index.php` gained matching `$typeColors` + `$typeLabels` entries and legend items for the 8 new types (labels "Control Re-test", "Finding Remediation", "Vendor Certification", "Vendor Contract", "Policy Attestation", "SSP Review", "Asset Review", "KRI Measurement"). **Read-only view — no schema migration and no new notification type.** Covered by `tests/integration/calendar_events_db.php` (seeds one in-current-month event per new category and asserts the real `getEvents()` aggregation surfaces each with the correct type; wired into `.github/workflows/aegis-integration.yml`).
...
### Features
Notification preferences (29 toggle types — including `control_retest_due` "Control re-test due", `finding_remediation_overdue` "Audit finding remediation overdue", `vendor_cert_expiring` "Vendor certification expiring", `vendor_contract_expiring` "Vendor contract expiring", `kri_measurement_overdue` "KRI measurement overdue", `audit_schedule_overdue` "Audit overdue to start", `policy_attestation_overdue` "Policy attestation overdue", `ssp_review_overdue` "SSP review overdue", and `asset_review_overdue` "Asset review overdue" — plus a `__digest__` delivery row: immediate/daily/weekly + time); profile edit (name, email); change password.
