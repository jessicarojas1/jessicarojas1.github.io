...
### Features
Notification preferences (24 toggle types — including `control_retest_due` "Control re-test due", `finding_remediation_overdue` "Audit finding remediation overdue", `vendor_cert_expiring` "Vendor certification expiring", and `vendor_contract_expiring` "Vendor contract expiring" — plus a `__digest__` delivery row: immediate/daily/weekly + time); profile edit (name, email); change password.
