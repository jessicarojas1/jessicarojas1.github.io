...
### Features
Notification preferences (26 toggle types — including `control_retest_due` "Control re-test due", `finding_remediation_overdue` "Audit finding remediation overdue", `vendor_cert_expiring` "Vendor certification expiring", `vendor_contract_expiring` "Vendor contract expiring", `kri_measurement_overdue` "KRI measurement overdue", and `audit_schedule_overdue` "Audit overdue to start" — plus a `__digest__` delivery row: immediate/daily/weekly + time); profile edit (name, email); change password.
