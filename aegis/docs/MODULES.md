...
### Features
Notification preferences (23 toggle types — including `control_retest_due` "Control re-test due", `finding_remediation_overdue` "Audit finding remediation overdue", and `vendor_cert_expiring` "Vendor certification expiring" — plus a `__digest__` delivery row: immediate/daily/weekly + time); profile edit (name, email); change password.
