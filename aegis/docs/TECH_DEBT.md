...
| TD-6 | **No query/result caching layer.** No cache backend exists in `src/` (the `cache` hits in `src/` are HTTP cache headers and per-request static memoization, not a query cache). Dashboards re-run all aggregations on every load. | Dashboard/aggregation controllers; `CustomDashboard` widget queries (flagged in MODERNIZATION.md §5 "Deferred"). `src/Auth.php` only memoizes permissions *per request* (`$permCache`, line 3 / 336). | Heavy aggregate pages (compliance %, risk counts, multi-widget custom dashboards) issue many queries every load with no reuse. | Add a small TTL cache abstraction (APCu for single-node, or Postgres-table/Redis for shared) behind an interface; cache expensive read-only aggregates with short TTLs and explicit invalidation on writes. **Requires a cache-backend decision** (called out in MODERNIZATION.md §5). | **P2** |
