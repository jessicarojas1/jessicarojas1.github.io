...
| TD-5 | **ADDRESSED for the high-cardinality lists — shared pagination helper added.** New `src/Pagination.php` (`build()` → LIMIT/OFFSET window from a COUNT + `?page=`; `render()` → filter-preserving, CSP-safe `<a>` controls + "Showing X–Y of Z"). Applied to the **risk register**, **issue tracker**, **vendor**, **asset inventory**, **audit**, **threat register** and **POA&M** lists (COUNT on the same WHERE, then `LIMIT ? OFFSET ?`); the **activity/audit log** was already paginated (`AdminController`). The IAM user list stays client-side by design (KL-1). Browser-verified: correct page windows, last-page handling, and filters preserved across pages (`/risk?status=open&page=2`). | `src/Pagination.php`; `RiskController::index` + `views/risk/index.php`; `IssueController::index` + `views/issue/index.php`; pagination styles in `app.css`. | The two largest growth-unbounded user lists now fetch/render one 25-row page instead of every row. | Remaining list pages still `fetchAll` unbounded but are lower-cardinality (compliance controls are grouped per-package/bounded; evidence/incidents are not standalone flat lists). Apply the same 3-line pattern to any that grow. Consider keyset pagination if deep offsets become hot. | **P3** (core lists done) |
| TD-6 | **Query/result caching — PARTIALLY ADDRESSED.** A `src/Cache.php` TTL cache now exists (APCu-backed, tenant-namespaced, graceful pass-through when APCu is absent) and is applied to the heaviest org-wide read-only aggregates: the main dashboard stat counts / risk-distribution / compliance-by-package, and every custom-dashboard widget. Short TTL (`Cache::DEFAULT_TTL = 30s`) is used in place of explicit write-invalidation, so staleness is bounded with no busting logic. Per-user dashboard queries (pending approvals/questionnaires, alerts) are deliberately left uncached to avoid cross-user leakage under a tenant-only key. | `src/Cache.php`; `controllers/DashboardController.php`, `controllers/CustomDashboardController.php`. Remaining uncached: report aggregations, compliance %, and other list/aggregate controllers. | Most-visited aggregate pages now reuse results within the TTL window; the rest still recompute every load. | **(2) DONE** — `src/Cache.php` now has a pluggable backend: **Redis** (opt-in via `REDIS_URL` + the optional phpredis extension; a SHARED store, correct across multiple instances) → **APCu** (single node) → **pass-through**. No Composer dependency added. Redis selection + graceful fallback are unit-tested; the live-Redis path needs a Redis server to exercise. **(1) DONE** — caching extended to the compliance-rollup page (`ComplianceController::index`) and the executive report (`ReportController::executive`, ~13 aggregate round-trips cached as one block), alongside the dashboard/metrics/custom-dashboard paths already cached. Remaining: (3) targeted `Cache::forget()` where freshness must beat the 30s window. | **P3** |
| TD-7 | **ADDRESSED — rate limiting uses a shared Redis counter when configured.** `Security::checkRateLimit()` now takes a Redis fast-path (atomic `INCR` + window TTL + lockout flag) via the `Cache` shared-counter primitives, mirroring the DB semantics exactly (allow `login_attempts` per `window_seconds`, then lock out for `lockout_seconds`). APCu is deliberately NOT used here (a per-node counter would weaken cross-instance brute-force protection), so without Redis it falls through to the authoritative `rate_limits` DB store — unchanged. | `src/Security.php` `checkRateLimit()`/`resetRateLimit()`; `src/Cache.php` counter primitives. | DB-store hot-row contention is removed when Redis is configured; single-instance deploys keep the zero-dependency DB path. | Verified against live Postgres that the DB fallback still enforces (correct password blocked while locked; reset restores login). The live-Redis path needs a Redis server to exercise. | **P3** (residual is Redis runtime verification) |
...
| TD-9 | **RESOLVED — failed email sends are queued and retried.** A failed immediate send in `Mailer::sendFromSettings` is now enqueued into a new `email_queue` table (instead of being dropped); `scripts/drain_email_queue.php` retries due rows with exponential back-off (2,4,8,…,60 min, capped) and marks a row `failed` after `max_attempts`. The happy path still sends immediately, so latency-sensitive mail (password reset, MFA) is unaffected. Provisioned as the `aegis-cron-email` Render cron (every 5 min). | `src/Mailer.php` (`smtpConfig`/`queue`/`sendFromSettings`); `scripts/drain_email_queue.php`; `email_queue` table in `install.php` + `schema.sql` + `schema.full.sql`; `render.yaml`. | Transient SMTP failures no longer silently lose review reminders / attestation campaigns / notifications. | Verified against live Postgres: a failed send enqueues; the drainer retries with back-off (attempts++, next_attempt_at advanced) and gives up to `failed` after `max_attempts`. | **DONE** |
| TD-10 | **ADDRESSED — background cron provisioned in the deploy manifest.** `render.yaml` now defines five `cron` services (notifications hourly, webhooks per-minute, reports hourly, workflows every 15 min, metrics daily), each running the matching CLI script via `dockerCommand` on the same image + database. Shared secrets that must match the web process (JWT_SECRET via `fromService`; APP_ENCRYPTION_KEY / AUDIT_HMAC_KEY / SMTP_* via a new `aegis-secrets` env-var group) are wired so settings decryption and the audit-HMAC chain stay consistent across processes. | `render.yaml` (web + 5 cron + `aegis-secrets` group). | Scheduled jobs now run on the Render deployment instead of silently never firing. | **Operator actions remaining (can't be verified from the repo):** (1) Render `cron` requires a **paid** plan — services are set to `starter`; review cost or consolidate jobs. (2) Populate the `aegis-secrets` group once in the dashboard. (3) The per-minute webhook cron cold-starts a container each run — fine for low volume; swap to a long-running worker if webhook latency/volume warrants. Could not deploy-test against Render from this environment; YAML structure validated locally. | **P3** (residual is operator/cost decisions) |
| TD-11 | **Category/status colour tokenization — LARGELY ADDRESSED.** Two passes shipped: (1) defined 34 CSS tokens that views referenced via `var(--x)` but were defined nowhere (a latent theming bug — e.g. `var(--moderate)` with no fallback was invalid and dropped); (2) replaced 313 literal hex colours across 48 views with `var(--token)` using an exact-value map, so light mode is pixel-identical and dark mode now themes them. New tokens (`--high`, `--gold`, `--neutral-*`, `--moderate`, …) added with light+dark values. Verified in a real browser (light+dark) — see below. | `public/css/app.css` (token defs); 48 swept views. | Brand-accent / dark-mode theming now reaches the converted category/status colours; they live in one place. | **Intentional residual:** hex inside `<script>`, SVG `fill=`/`stroke=`, and PHP colour maps that feed Chart.js (e.g. `kri/view.php $ragColor`) are left as literal hex on purpose — a CSS `var()` string can't be parsed by JS/Chart.js or applied as an SVG presentation attribute, so converting them would break charts/SVG. These are correctly NOT tokenizable. | **P4** (residual is by-design) |
...
| TD-13 | **RESOLVED — Bootstrap stylesheet vendored; app is now fully self-contained.** The last external asset (the Bootstrap CSS from jsdelivr) was vendored to `public/vendor/bootstrap/bootstrap.min.css` and `layout.php` repointed at the local copy. The vendored file is **byte-identical** to the CDN one (its SHA-384 matches the SRI hash that was already pinned in markup), so the change is provably zero visual difference — verified in a real browser (the stylesheet loads + parses into `document.styleSheets` under the strict CSP, no SRI/CSP rejection). With nothing left on jsdelivr, `cdn.jsdelivr.net` was dropped from `style-src`, so **no CSP directive carries an external origin**. `check_ui.php`'s external-host allowlist is now empty, failing CI if any CDN `<link>`/`<script>` is reintroduced. | `views/layout.php:8`; `public/vendor/bootstrap/bootstrap.min.css`; `src/Security.php` (`style-src`); `scripts/check_ui.php` (empty allowlist). | n/a — closed. | Optional future: tree-shake the vendored Bootstrap CSS to the ~handful of utility classes actually used (`.row`, `.col-*`, `.d-flex`, spacing), since the full file is 227 KB and only a subset is referenced. Cosmetic size win, not correctness. | **DONE** |
