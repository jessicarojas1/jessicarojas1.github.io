...
| 2026-05-25 | Global full-text search (/search, 7 entity types); Approval template builder (UI + create/toggle); Security policy admin (password complexity, MFA, lockout, network); Profile edit + password change; Password reset flow (email token, SHA-256, expiry); Compliance package scorecard (printable PDF-ready); Vendor external assessment portal (public token link, standalone HTML); Bulk risk operations (status/owner/delete); Custom fields admin UI; Tags system (admin CRUD + entity tagging API); 003_phase3.sql migration sections 3.8–3.10 (password_reset_tokens, vendor_portal_tokens, tags, entity_tags); all 30+ new routes wired in index.php; layout.php nav updated (Search, Edit Profile, Security Policy, Custom Fields, Tags, Approval Templates) |
| 2026-05-25 | Policy attestation campaigns (sign-off workflow, progress tracking, My Attestations page, re-attest support); Risk appetite statement (6-category org tolerance editor, inline color-coded table, appetite summary bar on risk index); Control effectiveness testing (control_tests table, test history, pass/fail/partial results, effectiveness %, testing dashboard with overdue alerts, Test button on compliance package); Incident response playbooks (4-table schema, PlaybookController, step builder, attach-to-incident checklist with AJAX step completion, run tracking); Vendor contract management (contract lifecycle, expiry alerts, auto-renewal tracking, contracts list with value totals); Compliance gap analysis dashboard (cross-framework scorecard grid, top-100 gaps table, multi-framework control overlap report); migration 003_phase3.sql sections 3.11–3.15 |
