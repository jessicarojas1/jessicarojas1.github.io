...
|---|---|---|---|
| **Terraform state** | Remote backend (S3+DynamoDB) — configure it; `versions.tf` has no backend block | **Critical** | Backend backup / versioned state bucket |
...
- **Regional durability:** optionally add S3 cross-region replication to a second
  Object-Locked bucket and replicate the CMK grant.
