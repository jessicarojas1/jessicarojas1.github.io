<?php
/**
 * PAL Platform — REST API v1 (self-contained JSON handler).
 *
 ...
 * or call session_start() again. PAL_ROOT is defined.
 *
...
try {
    ...
    error_log('[PAL API] auth error: ' . $e->getMessage());
...
try {
    ...
    error_log('[PAL API] rate limit error: ' . $e->getMessage());
    $sendError('Server error', 500);
}
...
try {
    ...
    error_log('[PAL API] ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    $sendError('Server error', 500);
}
