...
define('PAL_START', microtime(true));
...
set_exception_handler(function (Throwable $e): void {
    error_log('[PAL] Uncaught: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    if (!headers_sent()) {
        http_response_code(500);
        header('Content-Type: text/html; charset=utf-8');
    }
    $msg = htmlspecialchars($e->getMessage(), ENT_QUOTES | ENT_HTML5, 'UTF-8');
    echo "<!DOCTYPE html><html><head><meta charset='utf-8'><title>PAL — Configuration Error</title>"
...
foreach (['.env.local', '.env'] as $envFile) {
    if (file_exists(PAL_ROOT . '/' . $envFile)) {
        foreach (file(PAL_ROOT . '/' . $envFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) as $line) {
...
spl_autoload_register(function (string $class): void {
    foreach ([PAL_ROOT . "/src/{$class}.php", PAL_ROOT . "/controllers/{$class}.php"] as $path) {
...
require_once PAL_ROOT . '/src/Branding.php';
require_once PAL_ROOT . '/src/Auth.php';
...
if ($uri === '/health' && $method === 'GET') {
    ...
    $diskFree = @disk_free_space(PAL_ROOT);
    http_response_code($dbOk ? 200 : 503);
    echo json_encode([
        'status'    => $dbOk ? 'healthy' : 'degraded',
        'service'   => 'pal-platform',
        'timestamp' => date('c'),
        'checks'    => [
...
if ($uri === '/api/docs' && $method === 'GET') {
    require_once PAL_ROOT . '/api/docs.php';
    exit;
}
if (str_starts_with($uri, '/api/')) {
    require_once PAL_ROOT . '/api/index.php';
    exit;
}
...
function dispatch(string $controller, string $action, array $params = []): void {
    $file = PAL_ROOT . "/controllers/{$controller}.php";
...
require PAL_ROOT . '/views/errors/404.php';
