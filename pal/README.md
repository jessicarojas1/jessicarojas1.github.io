...
An enterprise-grade **Process, Approval & Library (PAL)** platform: the authoritative
...
- **Database:** PostgreSQL (schema namespace `pal`).
...
The first boot runs `install.php`, which creates the `pal` schema, applies the schema +
...
```bash
createdb pal && psql pal -c "CREATE SCHEMA IF NOT EXISTS pal;"
