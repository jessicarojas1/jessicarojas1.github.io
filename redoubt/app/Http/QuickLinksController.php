...
/** Quick Links module (Annex H). Members view; quicklink.manage maintains. */
final class QuickLinksController
{
    public static function index(string $nonce): void
    {
        ...
        $isManager = Authorize::can($user, 'quicklink.manage', ['program_id' => $programId]);
        $items = QuickLinks::listForUser($user, $programId, $isManager);
        $can = ['manage' => $isManager];
        $csrf = Security::csrfToken();
        $NONCE = $nonce;
        require dirname(__DIR__) . '/Views/app_quicklinks.php';
    }
    ...
    public static function post(): void
    {
        ...
        Authorize::requirePermission($user, 'quicklink.manage', ['program_id' => $programId]);
        ...
        switch ((string) ($_POST['action'] ?? '')) {
            case 'create': QuickLinks::create($programId, $data, $actorId); break;
            case 'update': QuickLinks::update((int) ($_POST['id'] ?? 0), $programId, $data, $actorId); break;
            case 'delete': QuickLinks::delete((int) ($_POST['id'] ?? 0), $programId, $actorId); break;
            default: self::plain(400, 'Unknown action.'); return;
        }
        header('Location: /app/quick-links?program_id=' . $programId);
    }
    ...
    private static function ctx(): array
    {
        ...
        $user = Auth::user();
        if (!Db::isConfigured()) {
            self::plain(503, 'Quick Links require the database.');
            return [null, 0, []];
        }
        $programs = [];
